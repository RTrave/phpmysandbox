...
class MySBModule
{
    ...
    public function upgrade()
    {
        ...
        if ($run_version == -1 or $run_version == $this->module_helper->version)
            return;
    ...
    public function reqsatisfied()
    {
        ...
        foreach ($this->module_helper->require as $modname => $modvers) {
            ...
            if ($mod->module_helper->version <= ($modvers - 1))
...
class MySBModuleHelper
{
    ...
    {
        global $app;
        if (isset($app->cache_modules) and !empty($app->cache_modules))
            return $app->cache_modules;
        $app->cache_modules = array();
        if (is_dir(MySB_ROOTPATH . '/modules'))
            $mod_dirs = scandir(MySB_ROOTPATH . '/modules');
        else
            $app->ERR('MySBModuleHelper::load(): directory modules not found or readable.');
