...
class MySBGroup extends MySBObject
{
    ...
    public function __construct($id = -1, $data_group = array())
    {
        global $app;
        if ($id != -1) {
            $req_group = MySBDB::query(
                "SELECT * FROM " . MySB_DBPREFIX . "groups " .
                "WHERE id=" . $id,
...
class MySBGroupHelper
{
    ...
    public static function create($name, $comments, $is_default = false)
    {
        ...
        if (isset($data_checkname['name'])) {
            ...
            $req_newgroup = MySBDB::query(
                "INSERT INTO " . MySB_DBPREFIX . "groups " .
                "(id) VALUES " .
                "(" . $gdata['id'] . ")",
                "MySBGroupHelper::create($name)",
                false
            ...
            MySBDB::query(
                "ALTER TABLE " . MySB_DBPREFIX . "users ADD " .
                "g" . $new_group->id . " boolean",
                "MySBGroupHelper::create($name)"
            );
            MySBDB::query(
                "ALTER TABLE " . MySB_DBPREFIX . "roles ADD " .
                "g" . $new_group->id . " boolean",
                "MySBGroupHelper::create($name)"
            );
            ...
            if ($is_default == true)
                ...
                $ivalue = 0;
            MySBDB::query(
                "UPDATE " . MySB_DBPREFIX . "users SET " .
            ...
            MySBDB::query(
                "UPDATE " . MySB_DBPREFIX . "roles SET " .
                "g" . $new_group->id . "=0",
                "MySBGroupHelper::create($name)"
            );
        }
    ...
    public static function delete($name = '')
    {
        global $app;
        if ($name == '') {
            $app->LOG('MySBGroupHelper::delete(): no group name given');
            return;
        }
        $group_id = MySBGroupHelper::getIDByName($name);
        // else $group_id = $this->id;
        if ($group_id == '')
            return;
        MySBDB::query(
            "DELETE FROM " . MySB_DBPREFIX . "groups " .
            "WHERE id=" . $group_id,
            "MySBGroupHelper::delete($name)"
        );
        MySBDB::query(
            "ALTER TABLE " . MySB_DBPREFIX . "users " .
            "DROP g" . $group_id,
            "MySBGroupHelper::delete($name)"
        );
        MySBDB::query(
            "ALTER TABLE " . MySB_DBPREFIX . "roles " .
            "DROP g" . $group_id,
            "MySBGroupHelper::delete($name)"
        );
    ...
    {
        global $app;
        if (isset($app->cache_groups) and !empty($app->cache_groups))
            return $app->cache_groups;
        $app->cache_groups = array();
        $req_groups = MySBDB::query(
            "SELECT * FROM " . MySB_DBPREFIX . "groups " .
            "ORDER BY id",
     ...
     * @return  MySBGroup           array of MySBGroup
     */
